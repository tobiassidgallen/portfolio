...
const smoothScroll = (target, duration = 1) => {
    ...
    // For touch devices, use native smooth scrolling for better performance
...
// Setup navigation for header links
...
if (navLinksItems.length > 0) {
    navLinksItems.forEach(link => {
        link.addEventListener('click', () => {
            if (window.innerWidth <= 992 && mobileMenuBtn && mobileMenuBtn.classList.contains('active')) {
                toggleMenu();
            }
        });
    });
