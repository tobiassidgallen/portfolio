...
// Enhanced Navigation for header and footer links
function setupNavLinks(selector) {
    document.querySelectorAll(selector).forEach(link => {
        link.removeEventListener('click', handleNavLinkClick); // Remove existing listeners
...
// Add a check to ensure elements exist before adding listeners
if (mobileMenuBtn && navLinks) {
    mobileMenuBtn.removeEventListener('click', toggleMenu); // Remove existing listeners
    mobileMenuBtn.addEventListener('click', toggleMenu);

    // Close menu when clicking outside
    document.removeEventListener('click', handleClickOutsideMenu); // Remove existing listeners
    document.addEventListener('click', handleClickOutsideMenu);

    // Close menu on window resize
    window.removeEventListener('resize', handleResize); // Remove existing listeners
    window.addEventListener('resize', handleResize);
}

// Function to toggle the mobile menu
function toggleMenu() {
    if (mobileMenuBtn && navLinks) { // Add check inside toggleMenu as well
...
// Function to handle window resize for closing menu
...
// Setup navigation for header links and logo
setupNavLinks('.nav-link, .logo-link');
// Setup navigation for footer links
setupNavLinks('.footer-links a');

// Prevent scroll when mobile menu is open (add passive: false)
if (navLinks) {
    navLinks.removeEventListener('touchmove', handleMenuTouchMove); // Remove existing listeners
    navLinks.addEventListener('touchmove', handleMenuTouchMove, { passive: false });
}
...
// Initial setup for navigation links on page load
document.addEventListener('DOMContentLoaded', () => {
    setupNavLinks('.nav-link, .logo-link');
    setupNavLinks('.footer-links a');

    // Re-attach resize listener on DOM load
     if (mobileMenuBtn && navLinks) {
        window.removeEventListener('resize', handleResize); // Remove existing listeners
        window.addEventListener('resize', handleResize);
    }
